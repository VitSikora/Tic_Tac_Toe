...
def check_winner():
    ...
    if (board[0][0]["text"] == board[1][1]["text"] == board[2][2]["text"]
        ...
        for i in range(3):
            board[i][i].config(foreground=color_yellow, background=color_white)
        game_over = True
        return
    ...
    if (board[0][2]["text"] == board[1][1]["text"] == board[2][0]["text"]
        ...
        board[0][2].config(foreground=color_yellow, background=color_white)
        board[1][1].config(foreground=color_yellow, background=color_white)
        board[2][0].config(foreground=color_yellow, background=color_white)
        game_over = True
        return
...
def new_game():
    ...
    for row in range(3):
        for column in range(3):
            board[row][column].config(text="", foreground=color_light_blue , background=color_blue)
...
for row in range(3):
    for column in range(3):
        board[row][column] = tkinter.Button(frame, text="", font=("Consolas", 50, "bold"),
                                            background=color_blue, foreground=color_light_blue , width=4, height=1,
